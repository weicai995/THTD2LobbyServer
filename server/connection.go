...
// 发送消息 封装后实际使用方法
func (c *Connection) SendStringAsMessage(responseType string, msgString string) (err error) {

	response2 := Response{responseType, msgString}
	jsonbyte2, err3 := json.Marshal(response2)
	if err3 != nil {
		zlog.Error(err3)
	}

	//msgType 在 conn.go中可以查看
	message := NewMsg("outChan", 1, []byte(jsonbyte2))
	select {
	case c.outChan <- message:
	case <-c.closeChan:
		err = errors.New("ERR_CONNECTION_LOSS")
